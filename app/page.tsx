...
type ProSession = 'London' | 'New York' | 'Asia' | 'Overlap' | 'Any'
type ProSetupType = 'Breakout' | 'Reversal' | 'Pullback' | 'Trendline' | 'Support/Resistance' | 'Custom'
type EmotionState = 'calm' | 'focused' | 'fomo' | 'revenge' | 'tired'
type ReviewOutcome = 'unknown' | 'win' | 'loss' | 'no-trade' | 'saved-me'
type FollowedVerdict = 'yes' | 'partially' | 'no'
...
const PRO_STORAGE_KEY = 'edge-check-pro-v1'

const proSessionOptions: ProSession[] = ['London', 'New York', 'Asia', 'Overlap', 'Any']
const proSetupTypeOptions: ProSetupType[] = ['Breakout', 'Reversal', 'Pullback', 'Trendline', 'Support/Resistance', 'Custom']
const emotionOptions: Array<{ value: EmotionState; label: string }> = [
  { value: 'calm', label: 'Calm' },
  { value: 'focused', label: 'Focused' },
  { value: 'fomo', label: 'FOMO' },
  { value: 'revenge', label: 'Revenge mindset' },
...
function getEmotionGuidance(emotion: EmotionState) {
  if (emotion === 'fomo') return 'Emotional pressure is elevated. Read the verdict twice before entering.'
  if (emotion === 'revenge') return 'High-risk mindset detected. Standing aside may be the best trade.'
  if (emotion === 'tired') return 'Low-energy state. Slow down and keep execution simple.'
  if (emotion === 'focused') return 'Good state for execution. Keep the checklist honest.'
  return 'Good baseline state. Let the checklist do the filtering.'
}
...
function isPlaybook(value: unknown): value is Playbook {
  ...
  const candidate = value as Partial<Playbook>
  return (
...
function isReviewEntry(value: unknown): value is ReviewEntry {
  ...
  return (
    ...
    typeof candidate.followedVerdict === 'string' &&
    typeof candidate.note === 'string' &&
    typeof candidate.createdAt === 'string' &&
    typeof candidate.instrument === 'string' &&
    typeof candidate.session === 'string' &&
    typeof candidate.setupType === 'string' &&
    Array.isArray(candidate.missingRuleTexts)
  )
}
...
export default function Home() {
  ...
  const [proInstrument, setProInstrument] = useState('Gold')
  const [proSession, setProSession] = useState<ProSession>('London')
  const [proSetupType, setProSetupType] = useState<ProSetupType>('Breakout')
  ...
  const [reviewOutcome, setReviewOutcome] = useState<ReviewOutcome>('unknown')
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (typeof parsed.instrument === 'string') setProInstrument(parsed.instrument)
      if (parsed.session && proSessionOptions.includes(parsed.session)) setProSession(parsed.session)
      if (parsed.setupType && proSetupTypeOptions.includes(parsed.setupType)) setProSetupType(parsed.setupType)
      if (parsed.emotion && emotionOptions.some((item) => item.value === parsed.emotion)) setProEmotion(parsed.emotion)
      if (typeof parsed.proTimerSeconds === 'number') {
        setProTimerSeconds(parsed.proTimerSeconds)
        setProTimerLeft(parsed.proTimerSeconds)
      }
      if (Array.isArray(parsed.playbooks)) {
        setPlaybooks(parsed.playbooks.filter(isPlaybook).map((item) => ({
          ...item,
          rules: item.rules.filter(isStoredRule).map((rule: Partial<Rule> & { text: string }) => normalizeRule(rule)),
        })))
      }
      if (Array.isArray(parsed.reviews)) {
        setReviews(parsed.reviews.filter(isReviewEntry))
      }
      if (typeof parsed.riskAmount === 'number') setRiskAmount(parsed.riskAmount)
      if (typeof parsed.stopDistance === 'number') setStopDistance(parsed.stopDistance)
      if (typeof parsed.pointValue === 'number') setPointValue(parsed.pointValue)
    } catch {}
  }, [])
  ...
  const activeEmotionLabel =
    emotionOptions.find((item) => item.value === proEmotion)?.label ?? 'Calm'
  const emotionGuidance = getEmotionGuidance(proEmotion)
  const positionSize =
    stopDistance > 0 && pointValue > 0
      ? Math.max(riskAmount / (stopDistance * pointValue), 0).toFixed(2)
      : '0.00'
  const thresholdGap = Math.max(minScore - score, 0)
  const mainBlockerText = hasMissingRequired
    ? `${missingRequiredRules.length} Mandatory ${missingRequiredRules.length === 1 ? 'rule is' : 'rules are'} still missing`
    : !meetsMinScore
    ? `Below your threshold by ${thresholdGap}%`
    : score >= 75
    ? 'No structural blocker. The setup qualifies.'
    : `No hard blocker, but quality is still only ${scoreBand.label}`
  const savedMeCount = reviews.filter((entry) => entry.outcome === 'saved-me').length
  const respectedVerdictCount = reviews.filter((entry) => entry.followedVerdict === 'yes').length
  const mostMissedRule = Object.entries(
    reviews.reduce<Record<string, number>>((acc, entry) => {
      entry.missingRuleTexts.forEach((item) => {
  ...
  const savePlaybook = () => {
    ...
    setPlaybooks((prev) => [playbook, ...prev.filter((item) => item.name !== trimmed)].slice(0, 12))
  ...
  const loadPlaybook = (id: string) => {
    ...
    setRules(cloneRules(playbook.rules))
    setMinScore(playbook.minScore)
    setProInstrument(playbook.instrument)
    setProSession(playbook.session)
    setProSetupType(playbook.setupType)
  }
  ...
  const saveReview = () => {
    const entry: ReviewEntry = {
      ...
      session: proSession,
      setupType: proSetupType,
      missingRuleTexts: missingRequiredRules.map((rule) => rule.text),
    }

    setReviews((prev) => [entry, ...prev].slice(0, 50))
    setReviewOutcome('unknown')
          ...
          {mode === 'pro' && (
            <div className={`rounded-[24px] p-3 md:p-4 ${ui.card}`}>
              <div className="mb-3 flex flex-col gap-2 xl:flex-row xl:items-center xl:justify-between">
                <div>
                  <div className={`text-sm font-semibold ${ui.secondaryStrong}`}>Pro tools</div>
                  <p className={`mt-1 text-xs ${ui.muted}`}>
                    Built around the few things pro traders actually use repeatedly: setup context, playbooks, risk planning, and honest post-trade review.
                  </p>
                </div>

                <div className={`rounded-full border px-3 py-1 text-[11px] font-semibold ${qualificationStyles.badge}`}>
                  {proInstrument} • {proSession} • {proSetupType}
                </div>
              </div>

              <div className="grid gap-3 xl:grid-cols-[1.15fr_0.85fr]">
                <div className={`rounded-[22px] p-3 ${ui.innerCard}`}>
                  <div className={`mb-2 text-sm font-semibold ${ui.secondaryStrong}`}>Trade context and playbook</div>
                  <p className={`mb-3 text-xs ${ui.muted}`}>
                    Tag the setup first, then save the checklist you actually trust so you can reuse it quickly.
                  </p>
                  ...
                  <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
                    ...
                    <select
                      value={proSession}
                      onChange={(e) => setProSession(e.target.value as ProSession)}
                      className={`h-10 rounded-2xl px-3 text-sm outline-none transition ${ui.select}`}
                    >
                      {proSessionOptions.map((item) => (
                        <option key={item} value={item}>
                          Session: {item}
                        </option>
                      ))}
                    ...
                    <select
                      value={proSetupType}
                      onChange={(e) => setProSetupType(e.target.value as ProSetupType)}
                      className={`h-10 rounded-2xl px-3 text-sm outline-none transition ${ui.select}`}
                    >
                      {proSetupTypeOptions.map((item) => (
                        <option key={item} value={item}>
                          Setup: {item}
                        </option>
                      ))}
                  ...
                  <div className={`mt-3 rounded-2xl border px-4 py-3 text-sm ${ui.statBox}`}>
                    <div className={`text-[10px] uppercase tracking-[0.18em] ${ui.muted}`}>Mindset check</div>
                    <div className="mt-1 font-semibold">{activeEmotionLabel}</div>
                    <p className={`mt-1 text-xs ${ui.muted}`}>{emotionGuidance}</p>
                  </div>
                  ...
                  <div className="mt-3 grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]">
                    <input
                      ...
                      placeholder="Save current checklist as playbook"
                      className={`w-full rounded-2xl px-3 py-2.5 text-sm outline-none transition ${ui.input}`}
                    />
                    <button
                      type="button"
                  ...
                  <div className="mt-2 grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]">
                    <select
                      ...
                      className={`h-10 rounded-2xl px-3 text-sm outline-none transition ${ui.select}`}
                    >
                      <option value="">Load saved playbook...</option>
                      {playbooks.map((item) => (
                    ...
                    {selectedPlaybookId ? (
                      <button
                        type="button"
                        onClick={() => deletePlaybook(selectedPlaybookId)}
                        className={`rounded-2xl px-3 py-2 text-xs font-semibold transition ${ui.dangerBtn}`}
                      >
                        Delete
                      </button>
                ...
                <div className="space-y-3">
                  <div className={`rounded-[22px] p-3 ${ui.innerCard}`}>
                    ...
                    {hasMissingRequired && (
                      <div className={`mt-3 rounded-2xl border px-4 py-3 text-sm ${ui.statBox}`}>
                        <div className={`text-[10px] uppercase tracking-[0.18em] ${ui.muted}`}>Missing mandatory rules</div>
                    ...
                    <div className="mt-3 flex items-center gap-2">
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          setProTimerSeconds(next)
                          if (!proTimerActive) setProTimerLeft(next)
                        }}
                        className={`h-10 rounded-2xl px-3 text-sm outline-none transition ${ui.select}`}
                      >
                  ...
                  <div className={`rounded-[22px] p-3 ${ui.innerCard}`}>
                    <div className={`mb-2 text-sm font-semibold ${ui.secondaryStrong}`}>Risk plan</div>
                    <p className={`mb-3 text-xs ${ui.muted}`}>Quick position size check. Keep your broker specs in mind.</p>
              ...
              <div className="mt-3 grid gap-3 xl:grid-cols-[1.1fr_0.9fr]">
                <div className={`rounded-[22px] p-3 ${ui.innerCard}`}>
                  <div className={`mb-2 text-sm font-semibold ${ui.secondaryStrong}`}>Post-trade review</div>
                  <p className={`mb-3 text-xs ${ui.muted}`}>
                    Log only what matters: outcome, whether you followed the verdict, and one honest note.
                  </p>
                  ...
                  <textarea
                    ...
                    placeholder="One honest note about what happened."
                ...
                <div className={`rounded-[22px] p-3 ${ui.innerCard}`}>
                  <div className={`mb-2 text-sm font-semibold ${ui.secondaryStrong}`}>Discipline snapshot</div>
                  <p className={`mb-3 text-xs ${ui.muted}`}>A few numbers only. Enough to see whether the process is helping.</p>

                  <div className="grid grid-cols-3 gap-2">
                    <div className={`rounded-2xl border px-3 py-3 ${ui.statBox}`}>
                      <div className={`text-[10px] uppercase tracking-[0.18em] ${ui.muted}`}>Reviews</div>
                      <div className="mt-1 text-lg font-bold">{reviews.length}</div>
                    </div>
                    <div className={`rounded-2xl border px-3 py-3 ${ui.statBox}`}>
                      <div className={`text-[10px] uppercase tracking-[0.18em] ${ui.muted}`}>Saved me</div>
                      <div className="mt-1 text-lg font-bold">{savedMeCount}</div>
                    </div>
                    <div className={`rounded-2xl border px-3 py-3 ${ui.statBox}`}>
                      <div className={`text-[10px] uppercase tracking-[0.18em] ${ui.muted}`}>Respected</div>
                      <div className="mt-1 text-lg font-bold">{reviews.length ? `${respectedVerdictCount}/${reviews.length}` : '0/0'}</div>
                    </div>
                  </div>
                  ...
                  <div className={`mt-3 rounded-2xl border px-4 py-3 text-sm ${ui.statBox}`}>
                    ...
                    <div className="mt-1 font-semibold">
                      {mostMissedRule ? mostMissedRule[0] : 'No saved reviews yet.'}
                    </div>
                  </div>
                </div>
              </div>
